...
class NotificationService {
  ...
  Future<void> initialize() async {
    ...
    if (settings.authorizationStatus == AuthorizationStatus.authorized) {
      debugPrint('Driver granted notification permission');
    } else if (settings.authorizationStatus == AuthorizationStatus.provisional) {
      debugPrint('Driver granted provisional notification permission');
    } else {
  ...
  Future<void> _saveTokenToBackend(String token) async {
    try {
      // Don't try to save to backend if driver is not logged in
      final storage = _ref.read(secureStorageProvider);
      if (await storage.getAccessToken() == null) {
        debugPrint("Skipping FCM token save: Driver is not logged in.");
        return;
      }

      final dio = _ref.read(dioProvider);
      await dio.patch(
        ApiConstants.driverFcmToken,
        data: {'token': token},
      );
